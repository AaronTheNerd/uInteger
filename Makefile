...
beta: src/uInt-beta.hpp tests/unit_tests.cpp
	$(CC) $(CFLAGS) $(BETA_FLAG) $(PERF_FLAG) tests/unit_tests.cpp
...
beta_benchmark: tests/benchmark.cpp src/uInt-beta.hpp
	$(CC) $(CFLAGS) $(BETA_FLAG) $(PERF_FLAG) tests/benchmark.cpp
	./run $(N) $(F) > data/beta_benchmark.txt

all: baseline beta beta_benchmark baseline_benchmark
